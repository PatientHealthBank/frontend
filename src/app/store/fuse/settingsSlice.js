	...
	reducers: {
		...
		setDefaultSettings: (state, action) => {
			...
			if (Array.isArray(action.payload) || typeof action.payload == "string") {
				...
				else if (action.payload.includes("provider")) {
					settings = {
						layout: {
							style: 'layout1', // layout-1 layout-2 layout-3
							config: {} // checkout default layout configs at app/fuse-layouts for example  app/fuse-layouts/layout1/Layout1Config.js
						...
						theme: {
							main: 'provider',
							navbar: 'provider',
							toolbar: 'provider_toolbar',
							footer: 'provider'
						}
					}
				}
				else if (action.payload.includes("clinic")) {
					settings = {
						layout: {
							style: 'layout1', // layout-1 layout-2 layout-3
							config: {} // checkout default layout configs at app/fuse-layouts for example  app/fuse-layouts/layout1/Layout1Config.js
			...
			const defaults = generateSettings(state.defaults, settings);
			const themes =
