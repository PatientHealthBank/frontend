...
import NoteModel from 'app/main/apps/my-diary/model/NoteModel';
import NoteLabel from 'app/main/apps/my-diary/NoteLabel';
import NoteReminderLabel from 'app/main/apps/my-diary/NoteReminderLabel';
	...
	React.useEffect(() => {
		if (props.specialties) {
			...
			props.specialties.forEach(specialty => {
				Object.entries(labels).forEach(([key, label]) => {if(label.handle == specialty){ listSpecialties.push(label.id)}})
			})
			console.log(listSpecialties)
			handleLabelsChange(listSpecialties)
		}
	}, [])
...
NoteFormSpecialty.propTypes = {};
NoteFormSpecialty.defaultProps = {
	variant: 'edit',
	note: null
};

export default withRouter(NoteFormSpecialty);
